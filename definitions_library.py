...
def refresh_page(message):
    ...
    try:
        for i in range(on_library_page * globals.definitions_content,
                       globals.definitions_content * (on_library_page + 1)):
    ...
    if on_library_page != 0 and not on_last_page(message.chat.id):
        prev_page = types.InlineKeyboardButton('⬅', callback_data='prev_def')
        menu = types.InlineKeyboardButton('Menu', callback_data='menu')
        next_page = types.InlineKeyboardButton('➡', callback_data='next_def')
...
def print_curr_page(message):
    ...
    last_library_message = globals.bot.send_message(chat_id=message.chat.id, text=globals.definitions_library_title,
...
def update_curr_page(message):
    globals.bot.edit_message_text(chat_id=message.chat.id, message_id=message.message_id, text=globals.definitions_library_title,
                                  reply_markup=refresh_page(message))
